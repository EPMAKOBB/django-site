...
Для массовой загрузки подготовленных данных воспользуйтесь руководством по импортам предсгенерированных пулов. В нём описаны требования к CSV/JSON, запуск management-команды и сценарий загрузки через админку.

* [Подробная инструкция](docs/recsys/pregenerated_pools.md)
...
# Кандидат выпуска сезонов 2027

Ветка `release/exam-seasons-2027` содержит отдельный выпуск ежегодных экзаменов. Порядок переноса, результаты проверок и оставшиеся условия выкладки: [release_2027_candidate.md](docs/recsys/release_2027_candidate.md). Не сливать в `main` до согласованной выкладки: push туда запускает FTP-деплой.
