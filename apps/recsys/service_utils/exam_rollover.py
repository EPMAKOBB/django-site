...
def placement_plan(exam):
    ...
    for row in exam.type_transitions.select_related("source_type", "target_type"):
        ...
        if row.relation == TaskTypeTransition.Relation.EQUIVALENT:
            ...
            explicit_empty = row.relation == TaskTypeTransition.Relation.PARTIAL and row.allow_empty_source_pool
            if (not selected and not explicit_empty) or not selected.issubset(eligible):
