...
class ApplicationCreateView(FormView):
    ...
    def form_valid(self, form: ApplicationForm) -> Any:  # type: ignore[override]
        form.save()
        ok_msg = "Заявка успешно отправлена"
        if self.request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"message": ok_msg})
        # Не показываем флэш-сообщение, чтобы не дублировать заголовок на странице ok
        return super().form_valid(form)

    def get_success_url(self) -> str:  # type: ignore[override]
        url = str(self.success_url)
        join = "&" if "?" in url else "?"
        return f"{url}{join}ok=1"

    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:  # type: ignore[override]
        context = super().get_context_data(**kwargs)
        form: ApplicationForm = context.get("form")  # type: ignore[assignment]
        subjects_count = 0
        if form:
